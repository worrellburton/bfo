...
export default function TrialBalance() {
  ...
  function handleGeneratePDF() {
    ...
    const colHeaders = report.columns.map((c) => `<th style="text-align:right;padding:4px 8px;font-size:9px;color:${mutedColor};font-weight:600">${c}</th>`).join("");

    printWindow.document.write(`<!DOCTYPE html><html><head>
      <title>Trial Balance - Dec 31, ${selectedYear}</title>
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        th, td { padding: 3px 8px; font-size: 10px; }
        .label { text-align: left; }
        .value { text-align: right; font-variant-numeric: tabular-nums; }
        .bold td { font-weight: 600; color: ${textColor}; }
        .total td { border-top: 1px solid ${borderColor}; font-weight: 700; color: ${textColor}; }
      ...
      <div class="header">
        <h1>Trial Balance</h1>
        <p>As of December 31, ${selectedYear} &middot; Generated ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
      </div>
      <table>
        <thead><tr><th class="label">Account</th>${colHeaders}</tr></thead>
        <tbody>
        ${report.rows.map((row) => {
          ...
          const vals = row.values.map((v) => `<td class="value">${v ? formatCurrency(v) : ""}</td>`).join("");
